...
export function HomePage() {
  useDB();
  const blocks = db.all('homepageBlocks').filter((b) => b.enabled).sort((a, b) => a.order - b.order);
  return (
    <PublicShell>
      {blocks.length === 0 ? (
...
type ContentRow = { id: ID; title: string; slug: string; excerpt?: string; description?: string; content: string; thumbnail?: string | null; categoryId: ID | null; status: string; publishedAt: number | null; createdAt: number; authorId: ID; videoUrl?: string; tags?: string[]; eventDate?: string; eventTime?: string; location?: string; registrationUrl?: string; gallery?: string[] };
...
function ContentListPage({ table, title, icon, detailPath, showVideo }: {
  ...
  const rows = (db.where(table, (r) => (r as ContentRow).status === 'published') as unknown as ContentRow[])
...
function ContentDetailPage({ table, detailPath }: { table: 'articles' | 'news' | 'tutorials' | 'activities'; detailPath: string }) {
  ...
  const row = db.find(table, (r) => (r as ContentRow).slug === slug) as unknown as ContentRow | undefined;
  ...
  const author = db.byId('users', row.authorId);
  const related = (db.where(table, (r) => (r as ContentRow).status === 'published' && r.id !== row.id) as unknown as ContentRow[]).slice(0, 3);
  return (
    <PublicShell>
      <article className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3 text-sm text-base-500 anim-rise">
          <span className="flex items-center gap-2"><Avatar name={author?.name ?? '?'} src={author?.avatar} size={26} /> <b className="text-base-700 dark:text-base-200">{author?.name ?? '—'}</b></span>
...
export function PageView() {
  useDB();
  const { slug } = useParams();
  const page = db.find('pages', (p) => p.slug === slug && p.status === 'published');
...
export function AboutPage() {
  useDB();
  const team = useMemo(() => { try { return JSON.parse(getSetting('about_team', '[]')) as Array<{ name: string; role: string; photo?: string }>; } catch { return []; } }, [getSetting('about_team')]);
  const gallery = useMemo(() => { try { return JSON.parse(getSetting('about_gallery', '[]')) as string[]; } catch { return []; } }, [getSetting('about_gallery')]);
  return (
    <PublicShell>
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 grid-bg" />
        <div className="relative mx-auto max-w-4xl px-4 py-16 text-center sm:px-6 anim-rise">
          ...
          <h1 className="mt-3 font-display text-3xl sm:text-4xl font-bold text-base-900 dark:text-base-50">{getSetting('about_hero_title')}</h1>
          <p className="mx-auto mt-3 max-w-xl text-base text-base-500 dark:text-base-400">{getSetting('about_hero_subtitle')}</p>
        </div>
      </section>
      <section className="mx-auto max-w-4xl px-4 pb-4 sm:px-6">
        <Reveal><div className="card p-6 sm:p-8"><RichHTML html={getSetting('about_description')} /></div></Reveal>
      </section>
      <section className="mx-auto grid max-w-4xl gap-5 px-4 py-8 sm:px-6 sm:grid-cols-2">
        <Reveal><div className="card h-full p-6 border-t-4 border-t-brand-500">
          ...
          <div className="mt-3"><RichHTML html={getSetting('about_vision')} /></div>
        </div></Reveal>
        <Reveal delay={80}><div className="card h-full p-6 border-t-4 border-t-accent-400">
          ...
          <div className="mt-3"><RichHTML html={getSetting('about_mission')} /></div>
        </div></Reveal>
      </section>
      {getSetting('about_history') && (
        <section className="mx-auto max-w-4xl px-4 py-4 sm:px-6">
          <Reveal><div className="card p-6 sm:p-8">
            ...
            <RichHTML html={getSetting('about_history')} />
          </div></Reveal>
        </section>
      )}
      {getSetting('about_video') && youtubeId(getSetting('about_video')) && (
        <section className="mx-auto max-w-4xl px-4 py-6 sm:px-6"><Reveal><YouTube url={getSetting('about_video')} /></Reveal></section>
      )}
      {team.length > 0 && (
...
export function ContactPage() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    setSending(true);
    setTimeout(() => { sendContactMessage(form); setForm({ name: '', email: '', subject: '', body: '' }); setSending(false); toast('success', 'Pesan terkirim. Kami akan segera membalas.'); }, 500);
  };
  return (
    <PublicShell>
