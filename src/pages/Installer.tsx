...
import { db, seedInstaller } from '../lib/db';
import { hashPassword, makeSalt, genAppKey } from '../lib/services';
...
function checkRequirements(): Req[] {
  return [
    ...
    { label: 'Persistent Storage', detail: 'localStorage tersedia (MySQL via adapter)', ok: (() => { try { localStorage.setItem('__t', '1'); localStorage.removeItem('__t'); return true; } catch { return false; } })() },
...
export default function Installer() {
  ...
  if (db.meta().installed && !justInstalled) return <Navigate to="/" replace />;
  ...
  const runInstall = async () => {
    ...
    setStep(4);
    const salt = makeSalt();
    const passwordHash = await hashPassword(site.password, salt);
    ...
    await line(`> APP_KEY ${genAppKey().slice(0, 22)}… generated`);
    ...
    seedInstaller({
      siteName: site.siteName.trim(), siteUrl: site.siteUrl.trim(), slogan: site.slogan.trim(),
      adminName: site.adminName.trim(), adminEmail: site.adminEmail.trim(),
      passwordHash, salt, timezone: site.timezone, language: site.language, currency: site.currency,
    });
          ...
          {step === 2 && (
            <div className="w-full max-w-2xl card !bg-base-900/80 !border-base-800 p-6 anim-scale">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2"><Field label="Password"><TextInput type="password" value={dbForm.password} onChange={(e) => setDbForm({ ...dbForm, password: e.target.value })} className="!bg-base-925 !border-base-700 !text-base-100 font-mono" placeholder="••••••••" /></Field></div>
              </div>
          ...
          {step === 3 && (
            <div className="w-full max-w-2xl card !bg-base-900/80 !border-base-800 p-6 anim-scale">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                ...
                <Field label="Website URL" required><TextInput value={site.siteUrl} onChange={(e) => setSite({ ...site, siteUrl: e.target.value })} className="!bg-base-925 !border-base-700 !text-base-100 font-mono" /></Field>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <Field label="Admin Password" required error={errors.password} hint="Min. 8 karakter"><TextInput type="password" value={site.password} onChange={(e) => setSite({ ...site, password: e.target.value })} error={!!errors.password} className="!bg-base-925 !border-base-700 !text-base-100 font-mono" /></Field>
                <Field label="Confirm Password" required error={errors.confirm}><TextInput type="password" value={site.confirm} onChange={(e) => setSite({ ...site, confirm: e.target.value })} error={!!errors.confirm} className="!bg-base-925 !border-base-700 !text-base-100 font-mono" /></Field>
