...
import { login as doLogin, register as doRegister, changePassword, getSetting, MediaService, audit } from '../lib/services';
  ...
  return (
    <AuthFrame title="Reset Password" sub="Masukkan email terdaftar untuk menerima tautan reset.">
      {sent ? (
        ...
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); if (/^\S+@\S+\.\S+$/.test(email)) { setSent(true); toast('info', t('reset_sent')); } }}>
...
export function ProfilePage() {
  ...
  const save = () => {
    ...
    db.update('users', user.id, { name: form.name.trim(), phone: form.phone, bio: form.bio, instructorHeadline: form.headline });
    refreshUser();
    audit(user.id, user.name, 'update', 'user', user.id, 'Memperbarui profil');
  ...
  const savePw = async () => {
    ...
    setBusy(true);
    const err = await changePassword(user, pw.current, pw.next);
    setBusy(false);
  ...
  const uploadAvatar = async (f: File) => {
    ...
    db.update('users', user.id, { avatar: res.url });
    refreshUser();
