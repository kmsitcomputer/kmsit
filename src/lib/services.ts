...
export const getSetting = (key: string, fallback = ''): string => db.settings()[key] ?? fallback;
...
const SESSION_KEY = 'kmsit_session_token';
...
export const currentUser = (): User | null => {
  ...
  if (!s) return null;
  if (s.expiresAt < now()) { db.remove('sessions', s.id); localStorage.removeItem(SESSION_KEY); return null; }
  const u = db.byId('users', s.userId);
...
export async function login(email: string, password: string, remember: boolean): Promise<{ ok: true; user: User } | { ok: false; error: string }> {
  const u = db.find('users', (x) => x.email.toLowerCase() === email.trim().toLowerCase());
  if (!u) return { ok: false, error: 'Email tidak ditemukan.' };
  if (u.status === 'suspended') return { ok: false, error: 'Akun ditangguhkan. Hubungi administrator.' };
  const hash = await hashPassword(password, u.salt);
  if (hash !== u.passwordHash) return { ok: false, error: 'Password salah.' };
  db.all('sessions').filter((s) => s.userId === u.id && s.expiresAt < now()).forEach((s) => db.remove('sessions', s.id));
  const token = uid() + uid();
  db.insert('sessions', { token, userId: u.id, remember, expiresAt: now() + (remember ? 30 : 0.5) * 24 * 3600 * 1000 } as Omit<Session, 'id' | 'createdAt' | 'updatedAt'>);
  localStorage.setItem(SESSION_KEY, token);
  db.update('users', u.id, { lastLoginAt: now() });
  audit(u.id, u.name, 'login', 'user', u.id, 'Login berhasil');
  return { ok: true, user: { ...u, lastLoginAt: now() } };
}

export function logout(user: User | null) {
  const token = localStorage.getItem(SESSION_KEY);
  if (token) {
    const s = db.find('sessions', (x) => x.token === token);
  ...
  if (user) audit(user.id, user.name, 'logout', 'user', user.id, 'Logout');
}
...
export async function register(data: { name: string; email: string; password: string; role: RoleKey }): Promise<{ ok: true; user: User } | { ok: false; error: string }> {
  if (getSetting('allow_registration') !== '1') return { ok: false, error: 'Pendaftaran publik sedang dinonaktifkan.' };
  if (db.find('users', (x) => x.email.toLowerCase() === data.email.trim().toLowerCase()))
    return { ok: false, error: 'Email sudah terdaftar.' };
  const salt = makeSalt();
  const passwordHash = await hashPassword(data.password, salt);
  const user = db.insert('users', {
    name: data.name.trim(), email: data.email.trim().toLowerCase(), passwordHash, salt,
    roleKey: data.role === 'instructor' ? 'instructor' : 'student',
    status: 'active', avatar: null, bio: '', instructorApproved: data.role !== 'instructor', instructorHeadline: '',
  });
  audit(user.id, user.name, 'register', 'user', user.id, `Registrasi sebagai ${user.roleKey}`);
  notifyAdmins('Pendaftaran baru', `${user.name} mendaftar sebagai ${user.roleKey}.`, '/dashboard/users');
  const token = uid() + uid();
  db.insert('sessions', { token, userId: user.id, remember: true, expiresAt: now() + 30 * 24 * 3600 * 1000 } as Omit<Session, 'id' | 'createdAt' | 'updatedAt'>);
  localStorage.setItem(SESSION_KEY, token);
  return { ok: true, user };
}
