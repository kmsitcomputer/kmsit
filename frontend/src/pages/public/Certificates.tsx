  ...
  return (
    <div id="cert-print-area" className="relative mx-auto w-full max-w-[860px] overflow-hidden rounded-xl shadow-2xl" style={{ background: bg, aspectRatio: '1.414 / 1' }}>
      ...
      <div className="relative flex h-full flex-col items-center justify-center px-[7%] text-center" style={{ color: fg }}>
        ...
        <p className="mt-[1%] font-display font-bold" style={{ fontSize: compact ? 14 : 21, color: accent }}>{courseTitle ?? '—'}</p>

        <div className="mt-[3%] flex w-full items-end justify-between px-[4%]">
          <div className="text-center" style={{ minWidth: '24%' }}>
            <p className="font-display italic font-semibold" style={{ fontSize: compact ? 12 : 17, color: fg }}>{instructorName ?? '—'}</p>
...
export function VerifyPage() {
  ...
  const [searched, setSearched] = useState<ApiCertificate | null | 'invalid'>(null);
  const { number } = useParams();

  const verify = (value: string) => {
    const trimmed = value.trim();
    if (!trimmed) return;
    void api.verifyCertificate(trimmed).then((response) => setSearched(response.valid ? response.certificate : response.certificate)).catch(() => setSearched('invalid'));
    setNum(trimmed);
  };

  useEffect(() => { if (number) { setNum(number); verify(number); } }, [number]);
  const shown = searched && searched !== 'invalid' ? searched : null;
  const invalid = searched === 'invalid';
            ...
            <div className="anim-scale">
              ...
              {shown.status === 'issued' && (
                <div className="mx-auto mt-6 grid max-w-2xl gap-3 sm:grid-cols-3">
                  {[
                    { l: 'Pemegang', v: shown.studentName ?? '—' },
                    { l: 'Kelas', v: shown.courseTitle ?? '—' },
                    { l: 'Instructor', v: shown.instructorName ?? '—' },
                  ].map((x) => (
