...
import { db, type Order, type Payment, type Withdrawal, type WebhookLog, type ID } from '../../lib/db';
...
import { api, type ApiAdminWithdrawal, type ApiDigitalDelivery, type ApiWallet } from '../../lib/api';
...
export function OrdersPage({ own }: { own?: boolean }) {
  ...
  const [remoteRows, setRemoteRows] = useState<Order[]>([]);
  useEffect(() => { if (user) void api.orders().then(setRemoteRows).catch(() => setRemoteRows([])); }, [user]);
  if (!user) return null;
  let rows = remoteRows;
...
export function PaymentsPage() {
  ...
  useEffect(() => { if (user) void Promise.all([api.payments(), api.webhookLogs()]).then(([payments, logs]) => { setRows(payments); setWhLogs(logs); }).catch(() => { setRows([]); setWhLogs([]); }); }, [user]);
  if (!user) return null;
  return (
...
function WithdrawModal({ onClose, available = 0, onSubmitted = () => undefined }: { onClose: () => void; available?: number; onSubmitted?: () => void }) {
    ...
    <Modal open onClose={onClose} title="Ajukan Withdrawal" footer={
      <><button className="btn-ghost" onClick={onClose}>Batal</button>
        <button className="btn-primary" onClick={() => {
          ...
          }).catch((error) => setErr(error instanceof Error ? error.message : 'Gagal mengajukan withdrawal.'));
      ...
      <div className="mb-4 rounded-lg bg-brand-500/[0.07] border border-brand-500/25 px-4 py-3 text-sm">
        Saldo dapat ditarik: <b className="font-display text-brand-600 dark:text-brand-400">{fmtMoney(available)}</b>
...
export function WalletPage() {
  ...
  const [remote, setRemote] = useState<ApiWallet | null>(null);
  const refreshWallet = () => { if (user) void api.wallet().then(setRemote).catch(() => setRemote(null)); };
  useEffect(refreshWallet, [user]);
  if (!user) return null;
  const s = remote ? { balance: remote.summary.available, totalEarning: remote.summary.earned, totalGross: remote.summary.earned, pendingWd: remote.summary.reserved, totalPlatformFee: 0, withdrawable: remote.summary.available } : { balance: 0, totalEarning: 0, totalGross: 0, pendingWd: 0, totalPlatformFee: 0, withdrawable: 0 };
  const ledger = remote?.ledger ?? [];
  const withdrawals = remote?.withdrawals ?? [];
  return (
    <DashShell title="Dompet">
      ...
      {withdraw && <WithdrawModal available={s.withdrawable} onSubmitted={refreshWallet} onClose={() => setWithdraw(false)} />}
    </DashShell>
  );
}
...
export function MyDigitalPage() {
  const { user, toast } = useApp();
  const [deliveries, setDeliveries] = useState<ApiDigitalDelivery[]>([]);
  useEffect(() => { if (user) void api.digitalDeliveries().then(setDeliveries).catch(() => setDeliveries([])); }, [user]);
  if (!user) return null;
  return (
            ...
            return (
              <div key={d.id} className="card card-hover p-5 anim-rise" style={{ animationDelay: `${(i % 2) * 60}ms` }}>
                ...
                <div className="mt-3 flex gap-2">
                  {d.downloadUrl ? (
                    <a href={api.digitalDownloadUrl(d.id)} download target="_blank" rel="noopener noreferrer"
...
export function WithdrawalsPage() {
  ...
  const [withdraw, setWithdraw] = useState(false);
  const [remoteRows, setRemoteRows] = useState<ApiAdminWithdrawal[]>([]);
  const refreshWithdrawals = () => {
    if (!user) return;
    void (user.roleKey === 'instructor' ? api.wallet().then((result) => result.withdrawals) : api.adminWithdrawals()).then(setRemoteRows).catch(() => setRemoteRows([]));
  };
  useEffect(refreshWithdrawals, [user]);
  if (!user) return null;
  const isInstructor = user.roleKey === 'instructor';
  let rows: Withdrawal[] = remoteRows;
  ...
  return (
    <DashShell title="Withdrawal">
      ...
      <DataTable rows={rows} pageSize={9} searchKeys={(w: Withdrawal) => `${w.bankName} ${w.accountName} ${db.byId('users', w.userId)?.name ?? ''}`}
        ...
        columns={[
          ...(isInstructor ? [] : [{ key: 'who', label: 'Instructor', render: (w: Withdrawal) => { const name = (w as ApiAdminWithdrawal).userName ?? '—'; return <span className="flex items-center gap-2 text-sm font-semibold"><Avatar name={name} size={22} />{name}</span>; } }]),
          ...
          <><button className="btn-ghost" onClick={() => setProcess(null)}>Batal</button>
            <button className={process.status === 'rejected' ? 'btn-danger' : 'btn-primary'} onClick={() => {
              void api.updateWithdrawal(process.w.id, process.status, note).then(() => {
                toast('success', `Withdrawal ${process.status}.`); refreshWithdrawals(); setProcess(null);
